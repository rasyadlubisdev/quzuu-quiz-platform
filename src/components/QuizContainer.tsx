...
const QuizContainer: React.FC = () => {
      ...
      case "file":
        return (
          <FileAnswer
            ...
            isReviewMode={isReviewMode}
            onFileChange={(fileUrl) => handleUpdateAnswer(question.id, fileUrl)}
          />
        )
      case "truefalse":
        return (
      ...
      case "codeeditor":
        return (
          <CodeEditorAnswer
            userCode={userAnswer || ""}
            isReviewMode={isReviewMode}
            onCodeChange={(code) => handleUpdateAnswer(question.id, code)}
          />
        )
      default:
  ...
  const handleNavigate = (direction: "prev" | "next") => {
    if (direction === "prev" && currentNumber > 1) {
      router.push(`/event-details/1/start/analitika?num=${currentNumber - 1}`)
    }
        ...
        {isReviewMode && (
          <div className="mt-4 p-4 bg-yellow-50 border border-yellow-300 rounded">
            <p className="text-yellow-600 font-semibold">
              Anda sudah submit jawaban. Hasil di atas adalah review.
            </p>
          </div>
        )}
